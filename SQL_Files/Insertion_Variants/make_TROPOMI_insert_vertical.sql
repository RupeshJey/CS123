...
BEGIN 
    ...
    -- Find two farthest away points among first 30 entries in root (using vertical test)

    SELECT entry_id FROM (entry_pool NATURAL JOIN entry_geom) 
    WHERE mbr_tlc_lon = (SELECT MIN(mbr_tlc_lon) FROM (entry_pool NATURAL JOIN entry_geom)) LIMIT 1
    INTO e1; 

    SELECT entry_id FROM (entry_pool NATURAL JOIN entry_geom) 
    WHERE mbr_brc_lon = (SELECT MAX(mbr_brc_lon) FROM (entry_pool NATURAL JOIN entry_geom)) LIMIT 1
    INTO e2; 
