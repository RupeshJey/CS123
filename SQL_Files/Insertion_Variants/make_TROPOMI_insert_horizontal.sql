...
-- CREATE INDEX entry_pool_index ON entry_pool (entry_id);
...
BEGIN 
    ...
    -- Find two farthest away points among first 30 entries in root (using horizontal test)

    SELECT entry_id FROM (entry_pool NATURAL JOIN entry_geom) 
    WHERE mbr_brc_lat = (SELECT MIN(mbr_brc_lat) FROM (entry_pool NATURAL JOIN entry_geom)) LIMIT 1
    INTO e1; 
    SELECT entry_id FROM (entry_pool NATURAL JOIN entry_geom) 
    WHERE mbr_tlc_lat = (SELECT MAX(mbr_tlc_lat) FROM (entry_pool NATURAL JOIN entry_geom)) LIMIT 1
    INTO e2; 

    -- insert these into node entries
    IF (use_inner) THEN 
        -- UPDATE rtree_properties SET a = 1000;
    ...
    -- UPDATE rtree_properties SET a = GREATEST((SELECT COUNT(*) FROM entry_pool), a);
    ...
    --     DELETE FROM entry_pool LIMIT 1;
    
    -- END WHILE;

    

    DELETE FROM entry_pool;
