...
BEGIN 
    ...
    IF (use_inner) THEN 
        ...
        (e1, (SELECT child_node_id FROM (SELECT * FROM inner_node_entries) AS lne1 WHERE entry_id = e1 LIMIT 1), -1, 0),
        (e2, (SELECT child_node_id FROM (SELECT * FROM inner_node_entries) AS lne2 WHERE entry_id = e2 LIMIT 1), -1, 1); 
    ELSE 
        ...
        IF (use_inner) THEN 
            ...
            (curr_entry, 
                (SELECT child_node_id 
                 FROM (SELECT * FROM inner_node_entries) AS lne1 
    ...
    IF curr_level > 0 THEN 
        CALL update_MBR(
            (SELECT entry_id FROM 
                ...
                WHERE level = curr_level-1 AND child_node_id = curr_node)
        );
    END IF;
